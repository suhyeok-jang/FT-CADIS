...
class CIFAR10_Denoise_And_Classify(nn.Module):
    def __init__(self, vit_checkpoint=None):
        ...
        model, diffusion = create_model_and_diffusion(
            **args_to_dict(Improved_Diffusion_Args(), model_and_diffusion_defaults().keys())
        )
        ...
        if vit_checkpoint is not None:
            ...
            print('Successfully load fine-tuned weight')
        else:
            print('Use only off-the-shelf models')
...
class ImageNet_Denoise_And_Classify(nn.Module):
    def __init__(self, vit_checkpoint=None):
        super().__init__()
        model, diffusion = create_model_and_diffusion(
            **args_to_dict(Guided_Diffusion_Args(), model_and_diffusion_defaults().keys())
        )
        model.load_state_dict(
        ...
        if vit_checkpoint is not None:
            #Lora matrix 추가 -> Lora weight initialization
