"""
Mostly copy-paste from MeLo: Low-rank Adaptation is Better than Finetuning for Medical Image
...
class LoRA_ViT_huggingface(nn.Module):
    """Applies low-rank adaptation to a vision transformer for huggingface
    ...
    def __init__(self, vit_model: ViT, r: int, alpha: int, num_classes: int = 0, lora_layer=None):
        super(LoRA_ViT_huggingface, self).__init__()

        assert r > 0
        assert alpha > 0
        base_vit_dim = vit_model.vit.encoder.layer[0].attention.attention.query.in_features
        dim = base_vit_dim
        if lora_layer:
            self.lora_layer = lora_layer
        else:
            self.lora_layer = list(range(len(vit_model.vit.encoder.layer)))
        ...
        for t_layer_i, layer in enumerate(vit_model.vit.encoder.layer):
            # If we only want few lora layer instead of all
            if t_layer_i not in self.lora_layer:
                continue
            w_q_linear = layer.attention.attention.query
            w_v_linear = layer.attention.attention.value
            ...
            layer.attention.attention.query = _LoRALayer(w_q_linear, w_a_linear_q, w_b_linear_q, r, alpha)
            layer.attention.attention.value = _LoRALayer(w_v_linear, w_a_linear_v, w_b_linear_v, r, alpha)

        self.reset_parameters()
        self.lora_vit = vit_model
        if num_classes > 0:
            self.lora_vit.classifier = nn.Linear(vit_model.classifier.in_features, num_classes)
    ...
    def save_fc_parameters(self, filename: str) -> None:
        ...
        _in = self.lora_vit.classifier.in_features
        _out = self.lora_vit.classifier.out_features
        fc_tensors = {f"fc_{_in}in_{_out}out": self.lora_vit.classifier.weight}
    ...
    def load_fc_parameters(self, filename: str) -> None:
        ...
        with safe_open(filename, framework="pt") as f:
            ...
            try:
                ...
                self.lora_vit.classifier.weight = Parameter(saved_tensor)
            except ValueError:
...
class LoRA_ViT_timm(nn.Module):
    def __init__(
        ...
        # lets freeze first, except classifier (already ImageNet-1K finetuned)
